...
- `ListTaskArtifacts`：按 `ProjectFile.task_id + resource_type=artifact` 查询，关联 FileUpload 返回。
- `GetArtifact` / 下载：将 `artifact_id` 解释为 `FileUpload.public_id` 查询。
- 项目详情 Artifacts：按项目 ProjectFile 记录生成。
- 返回字段映射：
  - `artifact_id`、`file_public_id` → FileUpload.PublicID
  - 文件名、MIME、大小、SHA256 → FileUpload
  - 创建时间 → ProjectFile.CreatedAt
  - `artifact_type=file`、`status=completed`
  - 旧表独有的 description、version、export_format 保持空值或默认值
...
- 根据 `artifact_id`（即 `FileUpload.public_id`）和当前用户 org 查询 `FileUpload`。
- 使用 `FileUpload.StorageURI` 从对象存储获取文件。
- 不向前端暴露容器或宿主机绝对路径。
