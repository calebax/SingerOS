...
func TestGetProjectFileTree_FiltersByFileView(t *testing.T) {
	...
	tree, err := service.GetProjectFileTree(ownerCtx, project.PublicID, "", "")
	if err != nil {
	...
	tree, err = service.GetProjectFileTree(outsiderCtx, project.PublicID, "", "")
	if err != nil {
...
func TestBindProjectUserMembers_AdminCannotCreateOwner(t *testing.T) {
	...
	service := NewProjectService(database, nil, nil, "test").(*projectService)
